...
export default function PlannerView() {
  ...
  const executeAction = (action: PendingAction) => {
    ...
    if (action.kind === "remove") {
      ...
      updateTrip(action.tripId, {
        scheduled: {
          startMonth: action.month, startYear: action.year,
      ...
      updateTrip(action.tripId, {
        scheduled: { ...trip.scheduled, startMonth: action.month, startYear: action.year },
      });
      ...
      updateTrip(action.tripId, {
        scheduled: { ...trip.scheduled, endMonth: action.month, endYear: action.year },
      });
    }
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (!dragType || dragType === "new") {
      // Auto-schedule based on duration — no popup
      const span = suggestedMonthSpan(trip.durationWeeks);
      const end  = addMonths(month, year, span - 1);
      scheduleTrip(tripId, { startMonth: month, startYear: year, endMonth: end.month, endYear: end.year });
      return;
    }
